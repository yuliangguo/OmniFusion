...
class RandLANet(nn.Module):
    def __init__(self, d_in, num_classes, num_neighbors=16, decimation=4):
        ...
        self.encoder = nn.ModuleList([
            LocalFeatureAggregation(8, 32, num_neighbors),
            LocalFeatureAggregation(64, 64, num_neighbors),
        ...
        self.decoder = nn.ModuleList([
            ...
            SharedMLP(96, 8, **decoder_kwargs)
        ])

        # final semantic prediction
        self.fc_end = nn.Sequential(
            #SharedMLP(8, 64, bn=True, activation_fn=nn.ReLU()),
            #SharedMLP(16, 16, bn=True, activation_fn=nn.ReLU()),
            #nn.Dropout(),
    ...
    def forward(self, input):
        ...
        for lfa in self.encoder:
            ...
            encoder_stack.append(x.squeeze(-1).transpose(1,2).clone())
            coord_stack.append(coords[:,:N//decimation_ratio].clone())
            decimation_ratio *= d
            x = x[:,:,:N//decimation_ratio]
            #_, index = knn(coords, coords[:,:N//decimation_ratio])
            #index_stack.append(index.squeeze(-1))
        ...
        for mlp in self.decoder:
            ...
            #_, neighbors = knn(coords[:,:N//decimation_ratio], coords[:,:d*N//decimation_ratio])
        ...
        coords = coords[:, torch.argsort(permutation), :].to(x.device)

        scores = self.fc_end(x).squeeze(-1)
        return scores.transpose(1, 2) + coords
    ...
    def test_forward(self, input):
        ...
        for lfa in self.encoder:
            # at iteration i, x.shape = (B, N//(d**i), d_in)
            x = lfa(coords[:,:N//decimation_ratio], x)          
            x_stack.append(x.squeeze(-1).transpose(1,2).clone())         
            decimation_ratio *= d
            x = x[:,:,:N//decimation_ratio]
            coord_stack.append(coords[:,:N//decimation_ratio].clone())
            _, index = knn(coords, coords[:,:N//decimation_ratio])
        ...
        return x_stack, coord_stack, index_stack    
...
class LocalFeatureAggregation(nn.Module):
    ...
    def forward(self, coords, features):
        ...
        #knn = KNN(k=self.num_neighbors, transpose_mode=True)
        #knn_output = knn(coords, coords)
...
if __name__ == '__main__':
    ...
    cloud = torch.randn((1, 65535, 7), dtype=torch.float32, device=cuda0)
